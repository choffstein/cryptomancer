...
from cryptomancer.execution_handler.take_profit_order import TakeProfitOrder

from typing import Optional, Tuple

def take_profit(account_name: str, base: str, underlying: str, size: float, side: str, 
                    trigger_price: float) -> Tuple[float, Optional[float]]:
    ...
    with execution_scope(wait = True) as session:   
        logger.info(f'{base} | Setting Take Profit {side.upper()} {size:,.4f} @ {locale.currency(trigger_price, grouping = True)}')

        underlying_order = TakeProfitOrder(account = account,
                                        market = underlying,
                                        side = side,
                                        size = size,
    ...
    if len(order_status) == 0:
        ...
        # DEALT WITH SOME HOW
        logger.info(f'{base} | {side.upper()} {size:,.4} {underlying} FAILED')
        return (0, None)
        ...
        if order_status.status == "closed" or order_status == "triggered":
            filled_size = abs(order_status.filled_size)
            if filled_size > 1e-8:
